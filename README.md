# Engineering Intelligence Showcase 🚀

Welcome! I am a **Mechanical Engineer** and **Python Developer** specializing in bridging the gap between deep technical engineering, data science, and modern AI-driven automation.

This repository serves as a **public dashboard** for my private projects in the fields of **AI Orchestration, Signal Processing, and Process Automation**.

---

## 🏗️ Core Project Pillars

### 1. AI Orchestrator: Autonomous Agent Control (AI / Sales Ops)
*   **Project:** `AI_orchestrator`
*   **Problem:** Managing multiple AI agents and long-running tasks across different providers (Claude, Gemini, Codex) without manual intervention.
*   **Solution:** A robust Python-based orchestrator with a Markdown-based queue system, automated provider fallbacks, and a **Telegram integration** for remote control and approvals.
*   **Key Tech:** Python 3.10+, Telegram Bot API, Multi-Provider Routing, Task-Queue Management.
*   **Impact:** Fully autonomous execution of coding, review, and documentation tasks with human-in-the-loop safety layers.

### 2. EEG Analysis Suite (Data Science / MedTech)
*   **Project:** `eeg_analysis`
*   **Problem:** Processing and analyzing complex neurological time-series data requires high performance and rigorous quality standards.
*   **Solution:** A modern data science pipeline for EEG data processing, utilizing a strictly typed and tested Python environment.
*   **Key Tech:** Python (uv), SQLite, NumPy, Pandas, ruff/mypy/pytest CI.
*   **Impact:** Efficient handling and transformation of large-scale EEG datasets for research and diagnostic purposes.

### 3. Engineering Signal Processing: Zeitrohdaten (Mechanical Engineering)
*   **Project:** `Aufbereitung_Zeitrohdaten`
*   **Problem:** Raw vibration data from machinery is difficult to interpret without specialized transformation and visualization.
*   **Solution:** An automated tool for cleaning and transforming raw time-series data into actionable engineering charts, such as **Campbell diagrams**.
*   **Key Tech:** Python, Signal Processing (FFT), NumPy, Matplotlib.
*   **Impact:** Drastically reduced manual effort in vibration analysis and structural health monitoring.

### 4. AI-Driven Sales Automation (Sales Ops / AI)
*   **Project:** `Linked-Engineering_Vertrieb`
*   **Problem:** High manual effort in lead generation and CRM management for engineering consultancies.
*   **Solution:** A full-stack automation pipeline that parses leads using **Local LLMs (Ollama)** and manages them in a custom CRM.
*   **Key Tech:** Python 3.12, Ollama (Qwen2.5), Gmail API (OAuth 2.0), Flask, SQLite.

---

## 🧪 Tech Stack & Standards
*   **AI/ML:** Autonomous Agents, Local LLMs (Ollama), OpenAI/Anthropic APIs.
*   **Engineering:** Signal Processing, Vibration Analysis (NVH), Structural Dynamics.
*   **Software Quality:** `uv` (Package Management), `ruff` (Linting), `mypy` (Type Safety), `pytest` (Testing).

---

## 🚀 Work with Me
I focus on **efficient, maintainable code** that delivers **measurable business value**.

*   **Status:** Available for freelance projects in Python automation, AI integration, and technical data analysis.
*   **Access:** These projects contain proprietary IP and are kept in private repositories. **Code access for specific modules can be granted upon request** after an initial consultation.

📧 **Contact:** [Your Email / LinkedIn Profile]
